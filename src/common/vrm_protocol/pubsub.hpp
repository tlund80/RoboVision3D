#ifndef __VRM_PROTOCOL_PUBSUB_HPP__
#define __VRM_PROTOCOL_PUBSUB_HPP__
...
namespace vrm_protocol {
	...
	template<typename T>
	class pubsub_server : public server<T> {
	public:
	  ...
	  /** Destructor. */
	  virtual ~pubsub_server()
	  {}

	  virtual void startup(const std::string &addr = "tcp://127.0.0.1:6000")
	  {
		if (!network_entity::_s) {
			network_entity::_s = socket_ptr(new zmq::socket_t(*network_entity::_ctx_ptr, ZMQ_PUB));
		}
	  ...
	  template<typename T>
	  class pubsub_client : public client<T> {
	  public:
	    ...
	    {
			if (!network_entity::_s) {
				network_entity::_s = socket_ptr(new zmq::socket_t(*network_entity::_ctx_ptr, ZMQ_SUB));
			}
			try {
...
#endif //__VRM_PROTOCOL_PUBSUB_HPP__
