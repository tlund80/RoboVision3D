...
/////////////////////////////////////////////////////
void ReconstructPointCloud::MLSApproximation(pcl::PointCloud<PointT>::Ptr &cloud, pcl::PointCloud<PointT>::Ptr &target)
{
...
pcl::PolygonMesh ReconstructPointCloud::MarchingCubes(pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals, 
						        ...
						        bool useHoppe, double iso_level)
{
	//double leafSize = 0.5;
        //double isoLevel = 0.5;
	...
	// Set parameters
	mch.setGridResolution(grid_resolutionX,grid_resolutionY,grid_resolutionZ);
	mch.setIsoLevel(iso_level);   //ISO: must be between 0 and 1.0
	mch.setSearchMethod(tree2);
	mch.setInputCloud(cloud_with_normals);

	// Reconstruct
	pcl::console::print_highlight("Reconstructing Point Cloud using Hoppes MarchingCube algorithm!!\n");
	mch.reconstruct (triangles);

	}else{
	  
	  pcl::console::print_warn("MarchingCube not implemented yet!!");
	  // Initialize objects
/*	pcl::MarchingCubes<pcl::PointNormal> mc;

	// Set parameters
	mc.setGridResolution(grid_resolutionX,grid_resolutionY,grid_resolutionZ);
	mc.setIsoLevel(iso_level);   //ISO: must be between 0 and 1.0
	mc.setSearchMethod(tree2);
	mc.setInputCloud(cloud_with_normals);

	// Reconstruct
	pcl::console::print_highlight("Reconstructing Point Cloud using MarchingCube algorithm!!\n");
...
{
	double resolution = 0.0005;	//Set the size of the grid cell
